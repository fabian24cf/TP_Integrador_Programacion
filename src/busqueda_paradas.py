...
# Requiere que la lista esté ordenada de forma ascendente por distancia
# -----------------------------------------
def busqueda_binaria_distancia(paradas, distancia):
    """
    Busca una parada por distancia en una lista ordenada de forma ascendente.
    Divide el rango a la mitad en cada iteración. Muy eficiente (O(log n)).
    """
    izquierda = 0  # Índice inicial (bajo)
    derecha = len(paradas) - 1  # Índice final (alto)
...
def busqueda_interpolacion(paradas, distancia_objetivo):
    ...
    # paradas_ordenadas = bubble_sort_por_distancia(paradas.copy())  # Copia ordenada
    bajo = 0  # Índice inicial
    alto = len(paradas) - 1  # Índice final

    while bajo <= alto and distancia_objetivo >= paradas[bajo]["distancia"] and distancia_objetivo <= paradas[alto]["distancia"]:
        # Calcula posición estimada según la fórmula de interpolación
        pos = bajo + ((distancia_objetivo - paradas[bajo]["distancia"]) * (alto - bajo)) // \
                     (paradas[alto]["distancia"] - paradas[bajo]["distancia"])
        if paradas[pos]["distancia"] == distancia_objetivo:  # Encontró
            return paradas[pos]
        elif paradas[pos]["distancia"] < distancia_objetivo:  # Busca derecha
            bajo = pos + 1
        else:  # Busca izquierda
            alto = pos - 1
    return None  # No encontró nada
